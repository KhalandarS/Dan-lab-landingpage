...
const initialFileSystem: { [key: string]: FileSystemNode } = {
  'Agent8': {
    type: 'dir',
    children: {
      'info.txt': { type: 'file', content: 'Agent8: No-code AI agent builder.\nBuild agents using natural language prompts.\nStatus: LIVE BETA.' },
      'specs.md': { type: 'file', content: 'Architecture: Transformer-based cognitive engine.\nInput: Natural Language.\nOutput: Executable Agent Runtime.' }
    }
  },
  'Zerant': {
    type: 'dir',
    children: {
      'readme.txt': { type: 'file', content: 'Zerant: Agentic Mobile Browser.\nAutomates web navigation, form filling, and travel booking on mobile devices.' },
      'version.log': { type: 'file', content: 'v0.9.2 - Alpha\n- Added flight booking module\n- Fixed login persistence' }
    }
  },
  'Dapify': {
    type: 'dir',
    children: {
      'manifesto.txt': { type: 'file', content: 'Web3 development needs to be accessible.\nDapify allows users to generate smart contracts via prompt.\nTarget: Ethereum, Solana.' }
    }
  },
  'AI_Glasses': {
    type: 'dir',
    children: {
      'blueprint.asc': { type: 'file', content: '[RESTRICTED] Hardware Schematics...\nDisplay: MicroLED Transparent.\nProcessor: Neural Silicon v2.\nOS: RealityOS.' }
    }
  },
  'AGI_Research': {
    type: 'dir',
    children: {
      'journal.log': { type: 'file', content: 'Day 420: Model collapse observed in sector 7. Retraining with synthetic data.\nSelf-improvement loop stability: 87%.' },
      'hypothesis.txt': { type: 'file', content: 'The path to AGI lies not in parameter size, but in recursive reasoning architectures.' }
    }
  },
  'README.txt': { type: 'file', content: 'Welcome to DAN LAB Mainframe.\nUse "ls" to view files.\nUse "cd <folder>" to navigate.\nUse "cat <file)" to read files.\nType any other query to chat with the AI Kernel.' }
};
...
export const RetroComputer: React.FC = () => {
  ...
  // FIX: Prevent blank top scrolling
  useEffect(() => {
    const el = scrollRef.current;
    if (!el) return;

    // Enable scroll only if content exceeds screen
    setIsScrollable(el.scrollHeight > el.clientHeight + 3);

    // Lock to bottom when needed
    if (isLoading || history.length > 0) {
      el.scrollTop = el.scrollHeight;
    }

    // Lock top (prevents blank space)
    if (el.scrollTop < 0) el.scrollTop = 0;

  }, [history, isLoading, bootSequence]);
  ...
  const handleCommand = async (e: React.FormEvent) => {
    ...
    setHistory(prev => [...prev, { role: 'user', text: `${getPromptPath()} > ${rawInput}` }]);
    setInput("");
    ...
    if (cmd === 'ls') {
      ...
      if (node?.children) {
        ...
        responseText = files
          ...
          .join("  ");
      } else responseText = "Error: Cannot list contents.";
      ...
      if (!target) setCurrentPath([]);
        ...
        const p = [...currentPath, target];
        const node = resolvePath(p);
        if (node?.type === "dir") setCurrentPath(p);
        else setHistory(prev => [...prev, { role: 'model', text: `bash: cd: ${target}: No such directory` }]);
      }

    } else if (cmd === "cat") {
      if (!target) {
        setHistory(prev => [...prev, { role: 'model', text: "Usage: cat <filename>" }]);
      } else {
        const p = [...currentPath, target];
        const node = resolvePath(p);
        if (node?.type === "file") setHistory(prev => [...prev, { role: 'model', text: node.content || "" }]);
        else setHistory(prev => [...prev, { role: 'model', text: `cat: ${target}: No such file` }]);
      }

    } else if (cmd === "help") {
      setHistory(prev => [...prev, { role: 'model', text: "Available: ls, cd, cat, clear, whoami." }]);

    } else if (cmd === "clear") {
      setHistory([]);

    } else if (cmd === "whoami") {
      setHistory(prev => [...prev, { role: 'model', text: "guest@dan-lab-mainframe" }]);

    } else {
      setIsLoading(true);
      const aiResponse = await generateResponse(rawInput);
  ...
  return (
    ...
    >
      ...
      {/* RESTORED ORIGINAL 3D MACINTOSH LOOK */}
      <motion.div 
          ...
          <div className="w-full h-full bg-[#999] rounded-lg p-[4px] shadow-[inset_2px_2px_5px_rgba(0,0,0,0.2)]">
            ...
            >
              ...
              <div className="p-4 font-mono text-[#00FF41] text-xs leading-relaxed">
                ...
                {isLoading && <div className="animate-pulse mt-2">PROCESSING...</div>}
